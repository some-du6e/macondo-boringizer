# AGENT RULES MADE BY A HUMAN
1. falling back to shit is for FUCKING PUSSY AGENTS
    DONT FUCKING DO IT. 
1.5. IF THE SHIT IS DESTINED TO BE A POPUP. DONT FUCKING REDIRECT THE USER. TRY THE MOST TO NOT REDIRECT
1.75. DONT REFRESH THE PAGE
2. DONT OvER FUCK  OVER COMPLICATE ITTTTT
3. DONT GUESS RANDOM SHIT ABOUT THE PAGE WITH UR PUSSY ASS TEXT GUESSING METHODS. JSUT ASK THE USER FOR THE FUCKING HTML
4. WE ARE USING BUN AND EXTENSION.JS (`extension`) TO BUILD THIS SHIT
5. USE `bunx tsc --noEmit` TO CHECK YOUR CODE AND FIX IT IN A DAMN LOOP UNTIL ITS FINE
6. DO NOT TRY TO RUN THE SERVER!!



# AGENT DOCUMENTION
FOR POPUPS, COPY THE WORKING PATTERN EXACTLY:
    - inspect the HTML the user gave
    - click the hidden real action button like "Open your profile" / "Open the shop"
    - DO NOT click random visual areas like .donkey-area unless the HTML proves that is the actual opener
    - if one popup works, mirror that opener-finding pattern for the other popup


## docs
- extension.js
    - https://extensionjs.mintlify.app/llms.txt
- 
